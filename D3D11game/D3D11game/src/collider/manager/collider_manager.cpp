...
void ColliderManager::SetCollider(SquareCollider* obj, bool is_static)
{
	...
	spc.current_pos = Vec2(obj->GetStatus().center_pos.x, obj->GetStatus().center_pos.y);
...
void ColliderManager::CheckCollision()
{
	...
	{
		//ないオブジェクトを参照しないようにする
		//visual studio上で実行するとアクセス違反を例外処理せず、実行が停止されるので、
		//visual studio上でエラーが出ても気にしなくて大丈夫です。
		//exeからの実行では問題ないです。
		try
		{
			//volatileというには最適化させないための修飾子
			//[[maybe_unused]]属性というのは意図的に未使用の要素を定義していることを
			//コンパイラに伝え、警告を抑制するための属性
			[[maybe_unused]]volatile auto temp = func_queue.front().second.col->object->enabled;
			if (func_queue.front().second.col->object == nullptr)
			{
				throw;
			}
			func_queue.front().first(func_queue.front().second.col->object,
				Vec2(func_queue.front().second.col->GetStatus().center_pos.x, 
					func_queue.front().second.col->GetStatus().center_pos.y)
				- func_queue.front().second.prev_pos);
		}
		catch (...)
		{
#ifdef _DEBUG
	...
	for (auto&& sc : staticSquareColliderList)
	{
		sc.current_pos = Vec2(sc.col->GetStatus().center_pos.x, sc.col->GetStatus().center_pos.y);
	}
	for (auto&& dc : dynamicSquareColliderList)
	{
		dc.current_pos = Vec2(dc.col->GetStatus().center_pos.x, dc.col->GetStatus().center_pos.y);
	}
}
...
{
	auto col1_tag = col1.col->object->object_tag;
	auto col2_tag = col2.col->object->object_tag;
	if ((col1_tag == OBJECT_TAG::ENEMY||col1_tag==OBJECT_TAG::BOSS || 
		col1_tag == OBJECT_TAG::BOSS_BULLET || col1_tag == OBJECT_TAG::BOSS_ZONE || col1_tag == OBJECT_TAG::SELECT)&&
		(col2_tag == OBJECT_TAG::ENEMY || col2_tag == OBJECT_TAG::BOSS || 
			col2_tag == OBJECT_TAG::BOSS_BULLET || col2_tag == OBJECT_TAG::BOSS_ZONE || col2_tag == OBJECT_TAG::SELECT))
	{
		return false;
	}
	...
	*/
	...
	//◇◇このお互いの中心から中心の距離を求める
	float maxDistance = std::sqrt(status1.width * status1.width + status1.height * status1.height) +
		std::sqrt(status2.width * status2.width + status2.height * status2.height);
		...
		std::abs(status1.center_pos.y - status2.center_pos.y) < sum_height)
	{
		...
		//ベクトル
		auto col1vec = Vec2(col1.col->GetStatus().center_pos.x, col1.col->GetStatus().center_pos.y)
			- col1.prev_pos;
		auto col2vec = Vec2(col2.col->GetStatus().center_pos.x, col2.col->GetStatus().center_pos.y) 
			- col2.prev_pos;
