...
void Effect::SetParam(const size_t n, const size_t life, const float _speed)
{
	...
	for (size_t i = 0; i < num; ++i)
	{
		...
		param.push_back(Param(Transform(
			{ saki::random(-10.0f,10.0f),saki::random(-10.0f,10.0f),0 }, saki::vector3_zero<float>, scale*saki::random(0.8f, 1.2f)),
...
void Effect::SetScale(const Vec3& s)
{
	scale = s;
	for (auto&& p : param)
	{
		p.transform.set_scale(s);
	}
}