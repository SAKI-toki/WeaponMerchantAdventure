...
std::shared_ptr<ActionPattern> AttackAction::Action(std::shared_ptr<ActionPattern>& action_ptr, Boss& boss, const float center_pos_x)
{
	...
	else if (++time > interval)
	{
		...
		std::begin(bullet)->Init("buss_bullet", L"sample5.png", 32, 32, VEC2(boss.transform.pos));
	...
	{
		boss.gravity.DirectSetSidePower(saki::random(1.0f, 3.0f)*((num % 2 == 0) ? 1.0f : -1.0f));
	}
